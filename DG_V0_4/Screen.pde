      ...
      for(int py = y0; py <= y1; py++){
        ...
        if(d >= interno && d <= raio){
          ...
          if(op > 0.01){
            ...
            // Ha cruzamento quando outra onda (de dono diferente) ja passou por aqui
            // neste frame, as duas com intensidade suficiente.
            boolean cruzamento = donoGrelha[px][py] != -1 && donoGrelha[px][py] != dono
                                 && op > limiarCruz && opExist > limiarCruz;

            // FUNDO (fora do texto): o cruzamento deixa marca permanente de cor.
            if(textoEco[px][py] == 0 && cruzamento){
            ...
            // Onda transitoria (a animacao que passa por cima).
            ...
            // TEXTO:
            //  - pixel de letra branco -> a onda pinta-o de uma vez, com a cor cheia;
            //  - pixel ja pintado -> so um cruzamento lhe muda a cor, e misturando a
            //    partir da cor que ja la esta (nunca desaparece);
            //  - se voltar a branco (apagado), fica de novo disponivel para pintar.
            // O rasto da frase a sair (aApagarPix) e apagado pelas ondas.
            if(textoEco[px][py] > 0){
              if(!defOn[px][py]){
                grelhaRdef[px][py] = rPuro;
                grelhaGdef[px][py] = gPuro;
                grelhaBdef[px][py] = bPuro;
                defOn[px][py] = true;
              } else if(cruzamento){
                color corLetra = color(grelhaRdef[px][py], grelhaGdef[px][py], grelhaBdef[px][py]);
                color brilho = misturarCores(corLetra, color(rPuro, gPuro, bPuro), opExist, op);
                grelhaRdef[px][py] = int(red(brilho));
                grelhaGdef[px][py] = int(green(brilho));
                grelhaBdef[px][py] = int(blue(brilho));
              }
