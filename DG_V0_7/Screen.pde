class Screen {
  ...
  float[][] apagarProg;       // quanto cada pixel do rasto ja foi varrido pelas ondas
  float apagarLimiar = 1.2;   // exposicao a ondas necessaria para um pixel sumir de vez
  int contaApagar = 0;
  int ondasParaApagarMax = 35; // ondas a apagar ate forcar a transicao (limpa o resto e troca)
  ...
  Screen(PGraphics pg) {
    ...
    apagarProg = new float[colunas][linhas];
  ...
  void definirTexto(String frase){
    ...
    int bold    = 1;   // espessura extra (0 = normal, 1 = levemente mais grossas)
  ...
  void desenhar() {
    ...
    // troca quando o rasto ja foi todo apagado pelas ondas, ou ao fim de um numero
    // maximo de ondas (garante que ha sempre um momento de transicao para a frase nova)
    if(aApagar && (nadaParaApagar() || contaApagar >= ondasParaApagarMax)){
      proximaFrase();
    }
  }
  ...
  void iniciarApagar(){
    for(int x = 0; x < colunas; x++){
      for(int y = 0; y < linhas; y++){
        // so marca para apagar os pixeis ja revelados (os brancos nao precisam)
        if(textoEco[x][y] > 0 && defOn[x][y]){
          aApagarPix[x][y] = true;
          apagarProg[x][y] = 0;
        }
  ...
  // Passa para a frase seguinte. O que sobrar do rasto deixa de ser apagado e
  // fica integrado no fundo colorido (nunca branco); a nova frase entra por cima.
  void proximaFrase(){
    for(int x = 0; x < colunas; x++){
      for(int y = 0; y < linhas; y++){
        aApagarPix[x][y] = false;
      }
    }
        ...
        if(d >= interno && d <= raio){
          ...
          if(op > 0.01){
            ...
            if(forca > 0){
              ...
              // apagar = a onda VARRE o pixel para a SUA propria cor (nunca branco),
              // integrando a antiga letra no campo colorido. Quando ja foi varrido o
              // suficiente deixa de ser rasto, mas fica colorido (sem buraco branco).
              apagarProg[px][py] += op;
              color atual = defOn[px][py]
                ? color(grelhaRdef[px][py], grelhaGdef[px][py], grelhaBdef[px][py])
                : color(rPuro, gPuro, bPuro);
              color nova = lerpColor(atual, color(rPuro, gPuro, bPuro), constrain(op, 0, 1));
              grelhaRdef[px][py] = int(red(nova));
              grelhaGdef[px][py] = int(green(nova));
              grelhaBdef[px][py] = int(blue(nova));
              defOn[px][py] = true;
              if(apagarProg[px][py] >= apagarLimiar){
                aApagarPix[px][py] = false;   // varrido: fica integrado no fundo colorido
              }
            }
          }
        }
